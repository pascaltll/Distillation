# =============================================================================
# SCRIPT: FINE-TUNING WAVLM LARGE (ESTRATEGIA CORREGIDA: DESCONGELADO PARCIAL)
# =============================================================================
...
os.environ["COMET_PROJECT_NAME"] = "wavML-Large" 
...
# ------------------------------------------------
# 1. CONFIGURACIÓN (AJUSTADA PARA EVITAR OOM)
# ------------------------------------------------
class Config:
    ...
    OUTPUT_DIR = Path("wavlm_large_finetuned_v2") # Nueva carpeta v2
    
    SAMPLE_RATE = 16000
    MAX_DURATION = 8.0 # Mantenemos 8s para seguridad en VRAM
    TARGET_LEN = int(SAMPLE_RATE * MAX_DURATION)
    
    # --- AJUSTES CRÍTICOS DE MEMORIA ---
    # Al descongelar capas, el consumo se dispara.
    # Usamos batch 1 pero acumulamos 32 veces para simular un batch grande.
    BATCH_SIZE_PER_GPU = 16     
    GRADIENT_ACCUMULATION = 2   # 1 * 32 = Batch efectivo de 32 (Muy estable)
    
    LEARNING_RATE = 1e-5         # LR bajo para ajuste fino
    NUM_EPOCHS = 15              
...
class IEMOCAPDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        try:
            ...
            y = np.zeros(self.config.TARGET_LEN) # Fallback seguro
            
        # Recorte/Pad a 8 segundos
...
# ------------------------------------------------
# 6. MODELO LARGE & ESTRATEGIA DE DESCONGELADO PARCIAL
# ------------------------------------------------
print(f"🔄 Descargando {cfg.MODEL_NAME}...")
model = WavLMForSequenceClassification.from_pretrained(cfg.MODEL_NAME, num_labels=4)
model.to(DEVICE)

print("\n🔓 APLICANDO ESTRATEGIA DE RESCATE (Partial Unfreeze)...")

# 1. Congelar Feature Encoder (CNNs que procesan audio crudo - siempre congelado)
...
# Esto es crucial: permite que el modelo Large adapte su "razonamiento" final
# sin romper el conocimiento general que ya tiene.
print("   -> Descongelando capas 18, 19, 20, 21, 22, 23 del Encoder...")
for param in model.wavlm.encoder.layers[-6:].parameters():
...
training_args = TrainingArguments(
    ...
    report_to=["comet_ml"], 
    run_name="wavlm-LARGE-unfrozen-6layers", # Nombre actualizado
    
    # Batch & Steps (Configurado para Batch Efectivo de 32)
    ...
    weight_decay=0.01,
    warmup_ratio=0.1,
    max_grad_norm=1.0, # Evita saltos bruscos
...
print("\n🚀 INICIANDO ENTRENAMIENTO (WavLM LARGE - UNFROZEN)")
print("Nota: Esto será lento debido al Gradient Accumulation, pero es seguro.")
trainer.train()

trainer.save_model(cfg.OUTPUT_DIR)
processor.save_pretrained(cfg.OUTPUT_DIR)
print(f"\n✅ Modelo Large (V2) guardado en: {cfg.OUTPUT_DIR}")
