...
# =============================================================================
# ⚙️ CONFIGURACIÓN V2 (VERSIÓN HUBERT)
# =============================================================================
CONFIG = {
    'train_csv': 'processed_combined_teacher/combined_train.csv',
    'val_csv':   'processed_combined_teacher/combined_val.csv',
    
    # 📂 NUEVA CARPETA para no sobrescribir los datos de WavLM
    'results_dir': 'results_v2_hubert', 
    
    # 🎓 AQUÍ VA TU MAESTRO HUBERT
    # Cambia esto por la ruta de tu carpeta o el nombre en HuggingFace
    'teacher_path': 'hubert_base_finetuned', 
    
    # Los mismos alumnos para comparar manzanas con manzanas
    'students': {
    ...
    # 🧪 HIPERPARÁMETROS V2 (Mismos que con WavLM para ser justos)
    'temp': 2.0,          
    'alpha': 0.75,        
...
def log_detailed(epoch, loss, uar, class_acc, mode, student_name):
    # Guardamos en results_v2_hubert
    path = os.path.join(CONFIG['results_dir'], 'detailed_log_hubert.csv')
...
def run_epoch(accelerator, student, teacher, loader, optimizer, is_train, mode="Baseline"):
    ...
    for raw_audio, mel_spec, labels in pbar:
        with torch.set_grad_enabled(is_train):
            ...
            # --- DESTILACIÓN CON HUBERT ---
            with torch.no_grad():
                # HuBERT también devuelve logits (asegúrate de que tu modelo tenga head de clasificación)
                teacher_output = teacher(raw_audio)
                if hasattr(teacher_output, 'logits'):
                    teacher_logits = teacher_output.logits
                else:
                    teacher_logits = teacher_output # Fallback
...
def main():
    ...
    if accelerator.is_main_process:
        print(f"\n🚀 STARTING V2 WITH HUBERT TEACHER")
    ...
    if accelerator.is_main_process: print(f"🎓 Loading HuBERT Teacher from: {CONFIG['teacher_path']}")
    
    # Intentar cargar HuBERT
    try:
        # Carga local (Tu modelo finetuneado)
        teacher = AutoModelForAudioClassification.from_pretrained(CONFIG['teacher_path'], num_labels=4)
    except Exception as e:
        if accelerator.is_main_process: 
            print(f"⚠️ Error cargando local ({e}). Usando versión base de Facebook (SIN FINETUNE).")
            print("⚠️ ADVERTENCIA: Si usas el base sin finetune, la destilación será mala.")
        teacher = AutoModelForAudioClassification.from_pretrained("facebook/hubert-base-ls960", num_labels=4)
        
    teacher.eval()
    for p in teacher.parameters(): p.requires_grad = False

    for student_name, arch in CONFIG['students'].items():
        if accelerator.is_main_process: print(f"\n{'='*50}\n🤖 {student_name} (Teacher: HuBERT)\n{'='*50}")
        ...
        for epoch in range(CONFIG['epochs']):
            tl, _, _ = run_epoch(accelerator, student, teacher_p, train_dl, optimizer, True, "Dist_HuBERT")
            vl, vu, v_cls = run_epoch(accelerator, student, teacher_p, val_dl, optimizer, False, "Dist_HuBERT")
            
            if accelerator.is_main_process:
                print(f"Ep {epoch+1:02d} | Loss: {tl:.3f} | UAR: {vu:.1%} | [A:{v_cls[0]:.2f} H:{v_cls[1]:.2f} S:{v_cls[2]:.2f} N:{v_cls[3]:.2f}]")
                log_detailed(epoch, tl, vu, v_cls, 'Dist_HuBERT', student_name)
                
                if vu > best_uar:
                    best_uar = vu
                    torch.save(accelerator.unwrap_model(student).state_dict(), f"{CONFIG['results_dir']}/best_hubert_{student_name}.pth")
                    print("  💾 New Best!")
